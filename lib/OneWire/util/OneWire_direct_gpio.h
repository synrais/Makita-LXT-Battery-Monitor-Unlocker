...
// ─────────────────────────────────────────────────────────────────────────────
//  RP2040 direct Single-cycle IO (SIO) register access.
//
//  All GPIO state changes go through the SIO hardware block at 0xD0000000,
//  which resolves in a single CPU clock cycle — no API overhead, no jitter.
//
//  SIO register map (uint32 offsets from base):
//    base[0]  0x000  CPUID           (unused)
//    base[1]  0x004  GPIO_IN         — read all GPIO input levels
//    base[5]  0x014  GPIO_OUT_SET    — atomic set output HIGH  (1 = drive high)
//    base[6]  0x018  GPIO_OUT_CLR    — atomic set output LOW   (1 = drive low)
//    base[9]  0x024  GPIO_OE_SET     — enable output driver    (OUTPUT mode)
//    base[10] 0x028  GPIO_OE_CLR     — disable output driver   (INPUT  mode)
//
//  PIN_TO_BITMASK returns (1u << pin_number) so each macro targets exactly
//  the right bit across all 30 RP2040 GPIO pins.
//
//  Note: GPIO function mux must be set to SIO for direct access to work.
//  The Arduino pinMode() call in OneWire::begin() handles this automatically.
// ─────────────────────────────────────────────────────────────────────────────
#define PIN_TO_BASEREG(pin)             ((volatile uint32_t *)(0xD0000000u))
#define PIN_TO_BITMASK(pin)             (1u << (unsigned)(pin))
#define IO_REG_TYPE                     uint32_t
#define IO_REG_BASE_ATTR
#define IO_REG_MASK_ATTR
#define DIRECT_READ(base, mask)         ((base[1]  & (mask)) ? 1 : 0)
#define DIRECT_WRITE_HIGH(base, mask)   (base[5]   = (mask))
#define DIRECT_WRITE_LOW(base, mask)    (base[6]   = (mask))
#define DIRECT_MODE_OUTPUT(base, mask)  (base[9]   = (mask))
#define DIRECT_MODE_INPUT(base, mask)   (base[10]  = (mask))
// Pico SDK busy_wait_us gives accurate blocking delays within interrupt-disabled
// sections (uses hardware timer, not loop counting).
// Only available on arduino-pico; MBED RP2040 keeps its own delayMicroseconds.
