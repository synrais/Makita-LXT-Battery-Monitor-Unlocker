...
//
//  Timing notes for RP2040 @ 133 MHz (fallback/API mode):
...
//               NOT the flags field from nybbles 34/35.  flags happens to
//               also equal 0x1e = 30 for type 6, but that is coincidental.
...
void read_voltages_type6(float *v_pack, float *cells, int cell_count) {
    // Enter voltage-read state (no response bytes expected).
    {
        uint8_t cmd[] = { 0x10, 0x21 };
        uint8_t dummy[1] = {0};
        cmd_cc(cmd, sizeof(cmd), dummy, 0);
        delay(10);
    }
    // Read all 10 cells.
    {
        uint8_t cmd[] = { 0xD4 };
        uint8_t rsp[20] = {0};
        cmd_cc(cmd, sizeof(cmd), rsp, 20);
        float pack = 0;
        for (int i = 0; i < min(cell_count, 10); i++) {
            uint16_t raw = le16(&rsp[i * 2]);
            cells[i] = (6000.0f - raw / 10.0f) / 1000.0f;  // convert to V
            pack += cells[i];
        }
        *v_pack = pack;
    }
...
void print_report(const BatteryInfo &info, float v_pack, float *cells) {
    ...
    if ((info.type == 0 || info.type == 2 || info.type == 3) &&
        ...
        int soc = (ratio == 0) ? 0 : (ratio < 10) ? 1 : min((int)(ratio / 10), 7);
        print_sep();
